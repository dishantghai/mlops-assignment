...
**2. Selective schema value hints by DB, not by cardinality threshold.** The blanket ≤10-distinct rule added helpful hints for thrombosis (`Admission`) and toxicology (`label`), but also added noise for codebase_community and financial (regressing −2 and −1 questions respectively), while california_schools improved by +1. Net: −6.6 pp. With a per-DB allowlist (`thrombosis_prediction: [Admission, KCT]`, `toxicology: [label, bond_type]`), the latency benefit would be retained without the accuracy cost on unrelated DBs.
