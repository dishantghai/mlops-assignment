...
| 7 | ~60% of revise cycles driven by unknown categorical values (thrombosis `Admission='-'` for outpatient, toxicology `label='+'` for carcinogenic). If generate_sql now uses correct values, verify accepts on first try → avg LLM calls/agent drops from 3.15 toward ~2.0 → effective LLM RPS at 5 user RPS drops from 15.75 to ~10 → ρ ≈ 0.64 (stable). | Schema value hints (inline DDL comments for TEXT columns ≤10 distinct values, ≤4-char values only to avoid natural-language noise) will reduce revise rate in load pool. Three generic prompt rules added: string case-sensitivity, ORDER BY LIMIT 1 for superlatives, LIKE for timestamp matching. | `agent/schema.py` — TEXT columns ≤10 distinct, all values ≤4 chars, show all values → `"label" TEXT /* e.g. '+', '-' */`; `agent/prompts.py` — 3 generic SQL rules | 5 RPS P50: **1.02 s** (−84% vs Run 7), P95: **5.54 s** (−82%). Eval accuracy: 43.3% → 40.0% (−3.3 pp) — refined hints eliminated financial noise; prompt rules fixed case-sensitivity and superlative-query regressions. |
...
| 9 (+ schema hints) | 5 | **1.02 s** | **5.54 s** | closest to SLO; eval −3.3 pp |
...
**2. Per-DB selective schema value hints.** Refining the hint threshold from ≤25 chars to ≤4 chars (code-like values only) and showing all distinct values recovered the accuracy regression from −6.6 pp to −3.3 pp, while generic prompt rules (case-sensitivity, ORDER BY LIMIT 1 for superlatives, LIKE for timestamps) fixed systematic SQL generation errors. The residual −3.3 pp gap traces to codebase_community — no hints apply there, so it is LLM stochastic variance on a 30-question eval, not a hint artifact. A per-DB allowlist (`thrombosis_prediction: [Admission, KCT]`, `toxicology: [label, bond_type]`) would preserve the latency benefit with zero accuracy cost on unrelated DBs.
