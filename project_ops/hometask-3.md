...
https://github.com/GlebBerjoskin/mlops-assignment

analysts ask questions in English
the system runs SQL against an internal warehouse and returns rows.
You're to present it to technical leadership and they'll buy it if they see the whole system running with Qwen3-30B-A3B against BIRD-bench showing some decent quality and performance.
...
Disclaimer: You don't need to set up Kubernetes, build a frontend or productionize past the toy-infra level. The point of the whole assignment is to learn what each layer tells you.
...
There's an example launch script at scripts/start_vllm.sh to get you started - feel free to modify it or roll your own. The vLLM docs are your reference for the available flags.

What to do:
Start vLLM with your initial configuration.

Confirm the model loads, responds, and the output looks reasonable. Try firing 3-5 inputs from evals/eval_set.jsonl manually.

Find your config. You'll probably need to revisit this once you have your agent running.

Write down your configuration in REPORT.md and explain it.
...
CPU-only vLLM: run vLLM on CPU with a small stand-in model like Qwen/Qwen3-0.6B. See the CPU install docs for more details.
...
Latency, with percentiles. The dashboard should let you look at it during a load test and answer "is the system slow, and if so, where in the request lifecycle?"
Throughput, with percentiles where it makes sense. Tokens out, requests served, queues, generation rate. Pick what's actually useful for someone owning this serving stack.
KV cache. The metric (or metrics) that tell you whether you have headroom for more concurrency or you're about to evict.
...
The starter dashboard at infra/grafana/provisioning/dashboards/serving.json gives you 2 pre-built panels to build on - feel free to extend it in the Grafana UI or edit the JSON directly. The vLLM metrics docs describe what each metric means, and the Grafana docs are your reference for building panels.
...
Tip: this phase is pure agent logic - you don't need the H100 running to build the graph and draft prompts. See Developing without the H100. Do final prompt tuning against the real Qwen3-30B-A3B endpoint, though - behavior and tokenization differ between models.

What to do:
Implement the LLM-calling nodes in agent/graph.py. generate_sql_node is filled in as a worked example - verify, revise, and the route_after_verify router are yours, and each docstring spells out what the node must return. The mechanics (LLM client, graph wiring, execute, schema rendering) are scaffolded; the prompts and the verify/revise logic are the actual exercise.

Write the prompts in agent/prompts.py. Aim for it to fire on the obvious cases: SQL errored, zero rows when the question implies rows exist, returned columns clearly don't answer the question.

Wire the conditional edge in agent/graph.py so verify-false routes back into a revise step (which then re-executes). Cap the loop at 3-5 iterations.

Test interactively with 5 questions from evals/eval_set.jsonl. Confirm that at least one question triggers a revise.
...
What to do:
Sign up for a local Langfuse account at http://localhost:3001

Create a project, grab the public and secret keys.

Add them to .env.

Add the Langfuse callback handler to your LangGraph invocation:
...
Fire 10 questions through the agent.

Open Langfuse UI. Find a trace and inspect it. You should see a waterfall with generate_sql, verify, and (sometimes) revise as nested spans, each with its prompt, response, latency, and token count.

Tag your traces with metadata, you'll need it during Phase 6.
...
What to do:
Implement evals/run_eval.py. It should:
Read the eval set
...
Run baseline eval. Note, this will hit your vLLM endpoint with 30 questions × ~2 vLLM calls each = ~60 requests. Watch Grafana while it runs.

Look at the per-iteration pass rate. If iter 0 pass rate is the same as iter 3 pass rate, your agent architecture is doing nothing. If iter 3 is meaningfully higher, the loop is earning its keep.
...
What to do:
Run the load test against your current configuration:
uv run python load_test/driver.py --rps n --duration 300
Watch the Grafana dashboard while it runs.

Either you hit the SLO or you don't. If you hit it on the first try - cool, but still take an iteration where you push past it to find what actually breaks. The point of this phase is the diagnosis skill, not just the green check.

Diagnose. Look at the dashboard. Which metric moves first as load ramps? Where is the system spending its time? Form a specific hypothesis about what's holding you back. Don't guess at a fix until the hypothesis is grounded in something you can point at on the dashboard.

Change one thing, re-run and confirm in the dashboard that the metric you targeted actually moved. Then ask whether end-to-end latency moved with it as sometimes a metric improves and the SLO doesn't, which is its own lesson.

Iterate. Each iteration should produce: a one-line note in REPORT.md of the form "saw X → hypothesized Y → changed Z → result was W", and a Grafana screenshot. Three or four iterations is normal. If you're on iteration seven, you're probably guessing instead of reading metrics - stop and re-read the dashboard.

Run the eval set against your final configuration. Save to results/eval_after_tuning.json. If your tuning regressed quality, that's part of the writeup, analyze it.

Document the full cycle in REPORT.md: baseline numbers, the iterations you went through, the final numbers, whether quality survived.
...
Serving configuration (Phase 1), your chosen flags, one line of justification each.

Baseline eval results (Phase 5), overall pass rate, per-iteration pass rate, brief commentary.

Hitting the SLO (Phase 6), baseline performance vs. SLO, the iteration log, the final numbers.

Agent value, one paragraph. Did the loop actually help? How do you know? Cite the per-iteration pass rate.

What you'd do with more time, and be specific here! "Add Kubernetes" doesn't count.
